...
Computational commit checked against the manuscript: `7a1bb19e8b0c814c189f0054194aab037c593318` — see `RELEASE_CHECK.md` (with `RELEASE_CHECK_run.log` and `RELEASE_CHECK_sessionInfo.txt` when a clean-copy run was made). Later commits change documentation and the release record only — `git diff --stat 7a1bb19e8b0c814c189f0054194aab037c593318 HEAD` lists them — so the scripts, data and outputs are those of the checked commit; after any change to code or outputs the release check is rerun and this line is regenerated.
